...
void BuildLeft(const std::vector<int64_t>& data, std::string mask, size_t border);
void BuildRight(const std::vector<int64_t>& data, std::string msk, size_t border, int64_t begin);
...
/*Можно переформулировать задачу - каждому числу можно присвоить 0, 1 или -1
0 - не взяли число
1 - взяли число в первое множество
-1 - взяли число в первое множество
Тогда наша задача найти нетривиальную нулевую сумму
Можно сделать так - в одном потоке посчитать половину и сложить в мап. А вторую часть распараллелить,
 так как мы не пишем в мап, а только читаем из него. Можно заметить, что вторая часть будет исполняться
 быстрее левой за счёт кол-ва потоков. Поэтому их можно сбалансировать, сделав левую половину поменьше*/